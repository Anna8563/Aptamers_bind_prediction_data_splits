...
#  
# ### 1.2. Reuse Existing Project Functions (with Fallbacks)
# 
# Try to import existing functions, create fallbacks if needed
...
#  
# ### 1.4. Model Visualization and Parameter Counting Utilities
...
#   
def train_epoch(model, dataloader, optimizer, criterion, device, alpha=1.0, beta=1.0):
    """Train for one epoch."""
    model.train()
    total_loss = 0.0
    n_batches = 0
...
pattern = re.compile(r"split_0\d0_0\d0$")

threshold_dirs = sorted(
    d for d in splits_dir.glob("split_*")
    if d.is_dir() and pattern.fullmatch(d.name)
)

# Iterate through all thresholds
for threshold_path in tqdm(threshold_dirs, desc="Thresholds"):

    original_name = threshold_path.name

    # split_090_050 -> split_09_05
    threshold_name = re.sub(
        r"split_0(\d)0_0(\d)0",
        r"split_0\1_0\2",
        original_name
    )
    ...
    if threshold_name.startswith("split_05") or threshold_name.startswith("split_06"):
        tqdm.write(f"Skipping {threshold_name}")
        continue
    # Iterate through all split modes
    for split_mode in tqdm(split_modes, desc="Split mode"):
        ...
        try:
            splits = load_splits_with_threshold(split_mode, base_dir=threshold_path)
        ...
        threshold_dir_out = os.path.join(ARTIFACTS_DIR, "metrics", threshold_name)
        os.makedirs(threshold_dir_out, exist_ok=True)
        # Define paths for incremental saving (per split mode)
        screening_results_path = os.path.join(
            threshold_dir_out,
            f"comprehensive_screening_{split_mode}_results.csv"
        )

        screening_histories_path = os.path.join(
            threshold_dir_out,
            f"comprehensive_screening_{split_mode}_histories.json"
        ) 
        ...
        completed_configs =  {f"{threshold_name}::{split_mode}::{config}" for config in completed_configs}
        ...
        for key, value in split_histories.items():
            all_histories[f"{threshold_name}::{split_mode}::{key}"] = value
        ...
        for model_cfg in model_configs:
            ...
            for apt_name, apt_feat in apt_features_map.items():
                for prot_name, prot_feat in prot_features_map.items():
                    ...
                    unique_config_name = f"{threshold_name}::{split_mode}::{config_name}"
...
if len(all_screening_results) > 0:
    ...
    print("="*80)
    print(all_screening_df[["threshold", "split_mode", "model", "apt_encoder", "prot_encoder", "MCC mean", "MCC std", 
                            "ROC-AUC mean", "ROC-AUC std", "F1 mean"]].to_string(index=False))
    
    # Save combined results
    combined_path = os.path.join(threshold_dir_out, "comprehensive_screening_all_splits.csv")
